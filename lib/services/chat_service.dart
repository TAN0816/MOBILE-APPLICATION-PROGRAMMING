...
class ChatService {
    ...
    try {
      ...
      batch.update(chatRoomDocRef, {
        ...
        'users': FieldValue.arrayUnion(
            [senderId, receiverId]), // Add sender and receiver to users array
        'lastMessage': message,
      ...
      DocumentReference messageDocRef =
          chatRoomDocRef.collection('messages').doc(chatMessage.id);
  ...
  Future<ChatRoom> createChatRoom(
      String sellerId, String currentUserId, String chatRoomId) async {
    try {
      // Example: Creating a chat room document in Firestore
      DocumentReference<Map<String, dynamic>> chatRoomRef =
          _firestore.collection('chatRooms').doc(chatRoomId);
      ...
      Map<String, dynamic> chatRoomData = {
        ...
        'users': [
          sellerId,
          currentUserId
        ], // Include both sellerId and currentUserId
        'lastMessageTimestamp':
            Timestamp.now(), // Example: Timestamp for last message
  ...
  Future<void> deleteMessage(String chatRoomId, String messageId) async {
    try {
      ...
      QuerySnapshot latestMessagesSnapshot = await _firestore
          .collection('chatRooms')
          .doc(chatRoomId)
          .collection('messages')
      ...
      if (latestMessagesSnapshot.docs.isNotEmpty) {
        // Update the chat room document with the latest message details
        await _firestore.collection('chatRooms').doc(chatRoomId).update({
          'lastMessageTimestamp':
              latestMessagesSnapshot.docs.first['timestamp'],
          'lastMessage': latestMessagesSnapshot.docs.first['message'],
        });
      } else {
        // If no messages left, update with default values or empty strings
        await _firestore.collection('chatRooms').doc(chatRoomId).update({
